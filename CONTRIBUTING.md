...
2. JavaScriptの機能ソースを変えた場合は `npm run build` で `content.js` を更新します。
3. `npm run check` を実行します。
4. 表示を変えた場合は、Edge / Chromeのテスト用環境で実際の表示も確認します。

生成済みの `content.js` もリポジトリに含めます。辞書の再生成方法と第三者ライセンスは [vendor/README.md](vendor/README.md) を参照してください。
